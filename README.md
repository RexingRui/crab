...
- **起订 5 只**（`service.RegMinCrabs`）。按盒的档按 `quantity × pack_size` 折算只数，
  按只的档就是数量本身，按斤的档没法折算成只、整单跳过这条校验。
  只卡买家这条路径——卖家给熟客记一只也是正常业务，录单接口不受限制。
  起订量跟着 `GET /api/public/specs` 一起下发（`min_quantity`），页面不自己写死一个数字。
  8 只装一盒就过线了，所以这条实际是给「按只卖」的档兜底的。
...
选了盒数才露出来）、收齐收货信息后提交。合计旁边跟着「共 X 只」，起订量就有地方落。
几个刻意的选择：
