...
func TestRegistrationFlow(t *testing.T) {
	...
	status, resp, data := e.register(t, registerBody(token, sp.ID, 3, "13900139001"))
	if status != http.StatusOK || resp.Code != errs.CodeOK {
	...
	if want := sp.UnitPrice * 3; reg.PayableAmount != want {
	...
	if o.PayableAmount != sp.UnitPrice*3 {
...
func TestRegistrationIgnoresClientPrice(t *testing.T) {
	...
	body := registerBody(token, sp.ID, 2, "13900139002")
	// 往请求里塞满各种降价字段，全都该被无视。
	body["items"] = []map[string]any{{"spec_id": sp.ID, "quantity": 2, "unit_price": 1, "amount": 1}}
...
func TestRegistrationSameLinkIsIdempotent(t *testing.T) {
	...
	token := e.regLink(t, "")
	sp := e.publicSpecIDs(t)[0]
	body := registerBody(token, sp.ID, 1, "13900139003")
	...
	body["items"] = []map[string]any{{"spec_id": sp.ID, "quantity": 9}}
...
func TestRegistrationDuplicatePhone(t *testing.T) {
	...
	if status, resp, _ := e.register(t, registerBody(e.regLink(t, ""), sp.ID, 1, phone)); resp.Code != errs.CodeOK {
		t.Fatalf("首次登记失败: status=%d code=%d", status, resp.Code)
	}

	status, resp, _ := e.register(t, registerBody(e.regLink(t, ""), sp.ID, 1, phone))
...
func TestRegistrationBadToken(t *testing.T) {
	...
	for name, tk := range cases {
		t.Run(name, func(t *testing.T) {
			status, resp, _ := e.register(t, registerBody(tk, sp.ID, 1, "13900139005"))
...
func TestRegistrationExpiredToken(t *testing.T) {
	...
	status, resp, _ := e.register(t, registerBody(expired, sp.ID, 1, "13900139006"))
	if resp.Code != errs.CodeUnauthorized {
...
func TestRegistrationRejectsDisabledSpec(t *testing.T) {
	...
	status, resp, _ := e.register(t, registerBody(e.regLink(t, ""), sp.ID, 1, "13900139007"))
	if resp.Code != errs.CodeInvalidParam {
