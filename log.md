...
加在买家这条路径上（`checkMinCrabs`），卖家录单不受限制——给熟客记一只是正常业务。
只数按盒折算：`quantity × pack_size`；按只的档就是数量；按斤的档折不出只数，
整单跳过这条校验（论斤买本来就不论只）。

值得记一笔的是**这条规则在当前价目表下是个空转**：四档都是 8 只装，一盒就过线。
它真正生效的场景是价目表里出现「按只卖」的档——「设置」页本来就能加，加了这条就是活的。
所以没有把它写成套餐专用的逻辑，而是按「这一单一共多少只」来数。

起订量由 `GET /api/public/specs` 随价目表一起下发（`min_quantity`），页面不写死数字，
和 `male_count` 默认值那条是同一个规矩：默认值只有后端一份。
