...
- 在自由搭配页签上看不见套餐那边选了什么，合计却凭空多几百块。所以两类都选了东西时，
  合计上方列出分项（套餐一行、散买一行），常驻不随页签切换。
- 同一档在两边都选，明细会出来两行一模一样的 ×1，卖家配货时得自己把它们加起来。
  提交前按 spec_id 合成一条。合并时要把套餐那条的 `male_count` 显式写出来——
  它原本靠后端的「没传就一半一半」兜底，合并后总数变了，不写死就可能和分开时不一样。
