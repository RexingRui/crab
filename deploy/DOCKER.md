...
脚本会把每个地址、每条路径各打 5 次（`REPEAT=` 可调），所以**时好时坏也照得出来**——
...
| 同一地址握手 `3/5` 这种 | 443 上两个进程在抢（宿主机的 Nginx/Caddy 和容器里的 Caddy 都开着），或机器负载高握手超时 | `ss -lntp \| grep :443` 看是不是两个 PID；`docker stats` 看负载 |
