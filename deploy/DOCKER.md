...
**提交备案前先确认三件事**，不然等十几天才发现走不通：

1. **域名在国内注册商名下**。在 Namecheap / GoDaddy 国际站 / Cloudflare 买的域名，
   国内备案不受理，得先转入腾讯云或阿里云。
2. **后缀在可备案列表里**。以腾讯云备案系统实际能否提交为准。
3. **域名已完成实名认证**，且持有者信息与备案主体（个人 + 身份证）一致。

顺序上还有个坑：**未备案期间 Caddy 签不出证书**。`--profile proxy` 的 Caddy 走
ACME HTTP-01 验证，需要公网能访问本机 80 端口，而未备案域名的 80/443 正好被云厂商拦着。
所以 HTTPS 和小程序域名登记这两步都必须等备案下来，中间没有捷径
（DNS-01 验证能绕过 80 端口拿到证书，但访问照样被拦，解决不了问题）。

备案下来之前可以先这样自测：机器上 `curl localhost:8080/healthz` 验证后端，
小程序侧用开发者工具勾「不校验合法域名」连 IP 调试（`project.config.json` 里
`urlCheck: false` 已经配好，但**只对开发者工具生效**，体验版和正式版一律强制校验合法域名）。**别为了图快把 `ENV` 改成 `dev` 长期跑**——
